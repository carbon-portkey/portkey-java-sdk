...
public class RetrofitProvider implements IRetrofitProvider {
     ...
     *                for example: <a href="https://did-portkey-test.portkey.finance">https://did-portkey-test.portkey.finance</a>
     * @param clazz   Interface class that each method contains Retrofit's @Annotation
     ...
     * }}<br/>
     * {@code
     * API api = RetrofitProvider.getAPIService("http://example.your-backend.com",API.class);
     * System.out.println(api.getGreet("Carbon").execute().body().toString());
     * }
     * </p>
     */
    public static <API> API getAPIService(@NotNull String hostUrl, Class<API> clazz) throws AElfException {
    ...
    public static Retrofit createRetrofit(@NotNull String hostUrl) {
